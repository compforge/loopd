...
func TestConversationWatchOnlyWakesAddressedActor(t *testing.T) {
	...
	original := &conversationv1.Conversation{
		Spec: conversationv1.ConversationSpec{Participants: []conversationv1.ConversationParticipant{
			{Kind: "operator", Key: "a", LatestMessageID: "001"},
			{Kind: "operator", Key: "b", LatestMessageID: "002"},
		}},
	}
	...
	other.Spec.Participants[1].LatestMessageID = "003"
	...
	addressed.Spec.Participants[0].LatestMessageID = "004"
	...
	partial.Status.Listeners = []conversationv1.ConversationListener{{Kind: "operator", Key: "a", LastMessageID: "001"}}
	...
	drained.Status.Listeners[0].LastMessageID = "004"
...
func TestConversationReadAndListenUseDifferentVerbs(t *testing.T) {
	var read, listen int
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "POST /v1/conversations/conv/listen":
			listen++
			var request loopd.ListenRequest
			if err := json.NewDecoder(r.Body).Decode(&request); err != nil {
				t.Error(err)
			}
			if request.Actor.Key != "a" || request.Limit != 10 {
				t.Errorf("Listen request = %+v", request)
			}
			_ = json.NewEncoder(w).Encode(loopd.ListenResult{Messages: []loopd.Message{{ID: "003"}}, LastMessageID: "003"})
		default:
	...
	result, err := runtime.Loop.Conv.Listen(context.Background(), "conv", loopd.ListenRequest{
		Actor: loopd.ActorRef{Kind: loopd.RoleOperator, Key: "a"}, Limit: 10,
	})
	if err != nil || result.LastMessageID != "003" {
		t.Fatalf("Listen = %+v, %v", result, err)
	}
	if read != 1 || listen != 1 {
		t.Fatalf("read = %d, listen = %d", read, listen)
	}
}
