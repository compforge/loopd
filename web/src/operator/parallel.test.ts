...
const layout = (messages: Message[]) =>
  groupParallelMessages(messages).map((group) =>
    group.columns.map((column) => column.map((item) => item.id)),
  );
...
describe("parallel detail groups", () => {
  ...
  it("gives each actor a column within a connected overlap group", () => {
    expect(layout([message("a", 0, 20), message("b", 10, 40), message("c", 30, 50)])).toEqual([
      [["a"], ["b"], ["c"]],
    ]);
  });
  ...
  it("includes actor kind in column identity", () => {
    expect(
      layout([message("a", 0, 20), { ...message("b", 0, 20), source_key: "a", source_kind: "operator" }]),
    ).toEqual([[["a"], ["b"]]]);
  });
  ...
  it("does not infer overlap from missing dates or stretch reversed intervals", () => {
    ...
    expect(layout([invalid, message("a", 10, 0), message("b", 11, 12)])).toEqual([
      [["a"]],
      [["b"]],
      [["unknown"]],
    ]);
