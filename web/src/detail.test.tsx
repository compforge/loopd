...
it("resolves user input to its target without requiring an Operator answer", () => {
  expect(detailOrganizer({ source_kind: "user", source_key: "user", target_kind: "operator", target_key: "router" }))
    .toEqual({ kind: "operator", key: "router" });
  expect(detailOrganizer({ source_kind: "user", source_key: "user", target_kind: "operator", target_key: "interaction" }))
    .toEqual({ kind: "operator", key: "interaction" });
});

it("uses the sender for answers and human questions addressed to the user", () => {
  expect(detailOrganizer({ source_kind: "operator", source_key: "interaction", target_kind: "user", target_key: "user" }))
    .toEqual({ kind: "operator", key: "interaction" });
  expect(detailOrganizer({ source_kind: "operator/longhorizon/manager", source_key: "run", target_kind: "user", target_key: "user" }))
    .toEqual({ kind: "operator", key: "longhorizon" });
});

it("prefers an explicit Operator recipient over the author", () => {
  expect(detailOrganizer({ source_kind: "operator", source_key: "router", target_kind: "operator", target_key: "longhorizon" }))
    .toEqual({ kind: "operator", key: "longhorizon" });
});
...
it("does not invent an Operator for broadcasts from users or direct Harness messages", () => {
  ...
  expect(detailOrganizer({ source_kind: "user", source_key: "user", target_kind: "harness", target_key: "agentgo" })).toBeUndefined();
...
it("distinguishes custom roles with a shared Run key and renders persisted report errors", () => {
  const messages = ["manager", "executor", "auditor"].map((role): Message => ({
    status: "completed", id: role, task_id: "task", conversation_id: "work", source_kind: "operator/longhorizon/harness", source_key: `run-uid/${role}`,
...
it("renders persisted message output status independently of body content", () => {
  const message: Message = { status: "streaming", id: "summary", conversation_id: "work", task_id: "", source_kind: "harness", source_key: "summary", created_at: "", updated_at: "", content: {version: "1.1", biz: "chat", meta: {}, blocks: [{id: "text", type: "text", content: "Already visible"}]} };
