...
import { ActorKind, isOperatorKind, operatorOwner, operatorRole, type Actor } from "./model";
...
describe("aggregate actor identity", () => {
  it("classifies built-in participants and custom Operator roles", () => {
    const actors: Actor[] = [ActorKind.User, ActorKind.Operator, ActorKind.Harness, message.source_kind].map(
      (kind) => ({ kind, key: "same" }),
    );
  ...
  it("resolves role owners while keeping a Harness with the same key distinct", () => {
    ...
    expect(
      detailOrganizer({ ...message, source_kind: ActorKind.Harness, source_key: "longhorizon" }),
    ).toBeUndefined();
    expect(
      detailOrganizer({
        ...message,
        source_kind: ActorKind.User,
        target_kind: message.source_kind,
        target_key: "run",
      }),
