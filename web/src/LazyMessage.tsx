...
export function LazyMessage({ message, onLoad, children }: {
  message: Message; onLoad(message: Message): void; children: ReactNode;
}) {
  ...
  useEffect(() => {
    ...
    setError(undefined);
    void messageBodies.load(message.conversation_id, message.id, controller.signal)
      .then((value) => { if (!controller.signal.aborted) callback.current(value); })
  ...
  return <div ref={element} className={loaded ? "lazy-message" : "lazy-message unloaded"}>
    {children}
    {!loaded && error && <button className="load-history" type="button" title={error} onClick={() => setAttempt((value) => value + 1)}>正文加载失败，点击重试</button>}
  </div>;
}
