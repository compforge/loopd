import { listMessages, messagePageSize, type Message } from "./api";
...
export class MessagePoller {
  ...
  observe(id: string) { this.known.add(id); }
  ...
  async sync(signal: AbortSignal, onPage: (messages: Message[]) => void): Promise<void> {
    ...
    for (let i = 0; i < ids.length; i += messagePageSize) {
      ...
      const rows = await listMessages(this.conversationID, { ids: ids.slice(i, i + messagePageSize).join(",") }, signal);
      signal.throwIfAborted();
      onPage(rows);
    }
    do {
  ...
  older(signal?: AbortSignal): Promise<Message[]> {
    ...
    if (!this.olderPending) this.olderPending = this.readOlder(signal).finally(() => { this.olderPending = undefined; });
    return this.olderPending;
  }

  private async read(signal?: AbortSignal): Promise<Message[]> {
    const initial = !this.initialized;
    const rows = await listMessages(this.conversationID, initial ? { order: "desc" } : { after: this.after, order: "asc" }, signal);
    signal?.throwIfAborted();
