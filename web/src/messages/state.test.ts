...
import { applyMessageEvent, mergeMessage } from "./state";
...
const frame = (m: Message, event: Record<string, unknown>) => {
  ...
  return decodeMessageFrame(
    "data: " +
      JSON.stringify(event.op === "start" || event.op === "end" ? { message: m, event: patch } : patch),
  );
};
it("merges lazy bodies without losing live output or replacing equal-revision content with metadata", () => {
  ...
  const delta = frame(full, {
    op: "append",
    seq: 5,
    mask: "block.content",
    block: { id: "text", content: "x" },
  });
  ...
  expect(
    applyMessageEvent([newerInfo], frame(restored, { op: "start", seq: 5, model: restored.content }))[0]
      .content,
  ).toEqual(full.content);
});
describe("message-addressed delivery", () => {
  it("isolates equal actor, block IDs and seq across outputs and ignores duplicate deltas", () => {
    ...
    for (const m of [a, b]) {
      ...
      messages = applyMessageEvent(
        messages,
        frame(m, { op: "set", seq: 2, block: { id: "text", type: "text", content: m.id } }),
      );
    }
    const delta = frame(a, {
      op: "append",
      seq: 3,
      mask: "block.content",
      block: { id: "text", type: "text", content: "!" },
    });
  ...
  it("rejects mismatched envelope identity", () => {
    ...
    expect(() => applyMessageEvent([], { ...event, event: { ...event.event, stream_id: "b" } })).toThrow(
      "identity mismatch",
    );
  });
  it("keeps metadata for bare deltas and repairs with a newer snapshot", () => {
    ...
    const repaired = {
      ...a,
      revision: 5,
      content: { ...a.content!, blocks: [{ id: "text", type: "text", content: "recovered" }] },
    };
  ...
  it("requires a known message for bare deltas and accepts unaddressed heartbeats", () => {
    const delta = frame(message("missing"), {
      op: "set",
      seq: 2,
      block: { id: "text", type: "text", content: "x" },
    });
...
it("ending one message leaves other messages live and accepts later actors", () => {
  ...
  messages = applyMessageEvent(
    messages,
    frame(b, { op: "set", seq: 2, block: { id: "text", type: "text", content: "still speaking" } }),
  );
  expect(messages[0].status).toBe("completed");
