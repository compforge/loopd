...
import { decodeMessageFrame } from "../events";
import { type Message } from "../types";
import { applyMessageEvent } from "../state";
...
describe("Human messages", () => {
  it("routes equal block IDs to different Messages and ignores a stale snapshot", () => {
    const a = message("a"),
      b = message("b", "confirm");
    b.content!.blocks[0].status = "success";
    b.revision = 2;
    ...
    const old = decodeMessageFrame(
      `data: ${JSON.stringify({ message: stale, event: { stream_id: "b", op: "start", seq: 1, model: stale.content } })}`,
    );
  ...
  it("renders a reply without the original message loaded and preserves its selected label", () => {
    ...
    reply.source_kind = "user";
    reply.reply_to_id = "question";
    reply.content!.blocks = [
      {
        ...
        outcome: "success",
        value: "small",
        question: question("success", "small"),
      },
    ];
  ...
  it("keeps options visible for terminal questions and distinguishes cancellation from refusal", () => {
    ...
    block.confirm_label = "Deploy";
    block.decline_label = "Skip";
    let html = renderToStaticMarkup(<MessageBody message={m} onReply={() => {}} />);
    expect(html).toContain("Deploy");
    expect(html).toContain("Skip");
    expect(html).toContain("忽略 / 取消");
    ...
    m.content!.blocks = [
      {
        ...
        outcome: "success",
        value: "custom answer",
        question: question("success", "custom answer"),
      },
    ];
  ...
  it("shows the original question's selection without loading its reply", () => {
    const m = message("scope");
    m.content!.blocks[0].status = "success";
    m.content!.blocks[0].selected_value = "small";
    const html = renderToStaticMarkup(<MessageBody message={m} onReply={() => {}} />);
    expect(html).toContain("Small");
    expect(html).toContain('checked=""');
    expect(html).not.toContain("<button");
  ...
  it("renders a cancelled reply with its question but no invented selection", () => {
    ...
    m.source_kind = "user";
    m.content!.blocks = [
      { id: "human", type: "human_reply", outcome: "dismissed", question: question("dismissed") },
    ];
    const html = renderToStaticMarkup(<MessageBody message={m} onReply={() => {}} />);
    expect(html).toContain("Scope");
    expect(html).toContain("Small");
    expect(html).toContain("已忽略");
    expect(html).not.toContain('checked=""');
    expect(html).not.toContain("<button");
  });
...
it("renders mixed content blocks without interpreting plain text as Markdown or executing HTML", () => {
  ...
  m.content!.blocks = [
    ...
    { id: "tool", type: "tool", name: "Search", status: "completed", content: "Found result" },
  ];
...
it("renders malformed Human choices as an invalid message rather than crashing the component", () => {
  const value = message("malformed");
  value.content!.blocks[0].choices = [{ value: "one", label: { invalid: true } }];
  expect(renderToStaticMarkup(<MessageBody message={value} onReply={() => {}} />)).toContain(
    "消息内容无法显示",
  );
});
it("keeps an unknown business block visible without making it interactive", () => {
  const value = message("custom");
  value.content!.blocks = [{ id: "custom", type: "custom/report", content: { summary: "visible" } }];
  const html = renderToStaticMarkup(<MessageBody message={value} onReply={() => {}} />);
