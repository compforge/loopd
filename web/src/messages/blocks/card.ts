...
import { isHumanQuestion, type HumanQuestion } from "./human";
...
export function humanCard(message: Message): HumanCard | undefined {
  ...
  if (block.type === "ask" || block.type === "confirm") {
    if (!isHumanQuestion(block)) throw new Error("Invalid Human question block");
    ...
    return {
      ...
      editable: question.status === "pending",
    };
  }
  if (block.type === "human_reply" && message.reply_to_id) {
    const question = block.question;
    if (!isHumanQuestion(question)) return undefined;
    return {
      type: question.type,
      mode: "reply",
      question_id: message.reply_to_id,
      question,
      selected_value: question.selected_value,
      editable: false,
    };
