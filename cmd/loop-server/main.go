...
func run() error {
	...
	loopServer, err := server.New(server.Config{
		...
		Database:              server.DatabaseConfig{MessageInlineBlocks: config.messageInlineBlocks, ContentMaxBytes: config.contentMaxBytes, Driver: config.databaseDriver, DSN: config.databaseDSN},
