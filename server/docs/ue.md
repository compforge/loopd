...
主对话与右侧详情首次只发现最近一页消息元信息，以正序展示；用户点击“加载更早的消息”
才向前取一页。历史游标与新消息发现游标相互独立，加载旧历史不改变实时消息的位置。
详情的并行分组基于已加载消息的时间区间，补入历史后重新计算，不为补齐一个分组扫描全部历史。

元信息先绘制消息位置、作者与状态，正文进入视口附近才读取，跨两个面板限制并发。
已经由 SSE 或读取取得的同版本正文复用；晚到的旧快照不能覆盖新 revision，元信息更新也不能
清空同版本正文。正文失败可单独重试，不阻塞整页其他消息。
...
`reply_to_id` 保留关联与“查看所回复的消息”跳转，不为引用自动加载预览。用户点击的原消息
尚未加载时，定点读取并定位它，不扫描中间的历史页，也不推进历史或新消息发现游标。
...
提交后的执行与页面实时观察分离，不把浏览器请求的取消信号传给已接受的工作。
页面刷新、关闭或 SSE 断开只取消 Conv Listener；Operator 继续消费，HarnessRunner 继续驱动，
直到各自判断完成、失败或按执行策略结束。用户显式停止属于独立控制动作，当前页面尚未提供
通用停止入口；页面停止观察、Message End 与业务停止不是一回事。
...
页面只发现当前历史页的元信息，正文按展示范围读取；不自动遍历全部历史，也不为全历史排队取正文。
...
断线退避重连，从 SQL 恢复运行态快照再接 Redis。每次连接建立后，页面分批刷新已见消息的
元信息，并逐页发现新消息，每页立即合并；正文仍按需读取。这样补偿首次加载的时间差和
断线期间的变化，避免刚结束的消息及已完成卡片的交互更新被遗漏。
