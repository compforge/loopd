...
页面 API 在普通分页结果上附加展示读模型：`card` 区分 content、ask、confirm；交互卡片用
request/reply 区分问题与答复，并携带问题内容、选中值和可操作状态。普通 content 卡片保留
AgentUE blocks：text 展示原文，markdown 渲染 Markdown，tool 保留工具信息；不因消息含有引用
...
直接引用按会话分组、去重并批量查询，已在结果内的消息直接复用，不递归展开回复链。读取范围
与当前消息的可见会话一致，不能借引用跨会话读取。原消息不存在时，保留当前消息及自身内容。
已答复问题通过正式 human_reply 的精确引用和参与者身份补齐最终选择，答复可位于其他分页；
普通发言不能伪造这项结果。数据库故障不伪装成引用不存在。

分页历史、Human 响应与 SSE 消息使用同一套投影。答复响应同时返回更新后的问题和新答复，页面
