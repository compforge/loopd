...
func TestActorsConsumeCompletedSpeechIndependently(t *testing.T) {
	...
	if err := poll.Commit(ctx, "conv", contract.CommitRequest{Actor: b, Through: inbox.Position}); err != nil {
		t.Fatal(err)
	}
	other, err = poll.Poll(ctx, "conv", contract.PollRequest{Actor: c})
...
func TestPollUsesTargetedSQLHistory(t *testing.T) {
	...
	if result.Messages[0].TargetKind != contract.ActorKindOperator || result.Messages[0].TargetKey != "a" {
		t.Fatal("target lost in public message mapping")
	}
	repeated, err := poll.Poll(ctx, "conv", contract.PollRequest{Actor: a, Limit: 2})
	if err != nil || repeated.Messages[0].ID != result.Messages[0].ID {
		t.Fatalf("uncommitted replay: %+v %v", repeated, err)
	}
	result, err = poll.Poll(ctx, "conv", contract.PollRequest{Actor: a, Limit: 2, After: result.Position})
	if err != nil || len(result.Messages) != 1 || result.Messages[0].ID != "006" {
		t.Fatalf("next batch = %+v, %v", result, err)
	}
	if err := poll.Commit(ctx, "conv", contract.CommitRequest{Actor: a, Through: result.Position}); err != nil {
		t.Fatal(err)
	}
	result, err = poll.Poll(ctx, "conv", contract.PollRequest{Actor: a})
	if err != nil || len(result.Messages) != 0 || result.Position != "006" {
		t.Fatalf("drained = %+v, %v", result, err)
	}
	other, err := poll.Poll(ctx, "conv", contract.PollRequest{Actor: b})
